...
* Fix: single CPT pages with JE queries whose Dynamic Args read `get_queried_object()` are similarly fixed. Previously the AJAX path set only `$GLOBALS['post']` (via `setup_postdata`), not `$wp_query->queried_object` — `get_queried_object()` reads from the latter, so JE macros bound to the queried post returned null on AJAX. The new context snapshot covers `is_singular` + post-typed `queried_object` so singular pages work the same way as archives.
* Fix: `[jsf_etch_count]` shortcode no longer flashes "0" before the real value on initial page load when the shortcode renders above the loop in source order (header counter, hero subtitle, sticky topbar, sidebar). Previously the shortcode could only resolve synchronously via `jet_smart_filters()->query->get_query_props()`, populated by JSF's `the_posts` priority 999 hook — so a shortcode rendered BEFORE the loop had nothing to read and fell back to `placeholder='0'`, with JS then patching the span on DOMContentLoaded ("0 → N" flash). Bridge now resolves in two passes: shortcode emits a sentinel when synchronous resolution fails, and a page-wide output buffer engaged on `template_redirect` substitutes the now-known value before the response leaves PHP. Cheap `strpos` early-out so pages without the shortcode pay zero extra cost. New filter: `apply_filters( 'jqbeb_count_late_substitution_enabled', true )` for sites with custom output-streaming setups.
* Fix: `.jsf-etch-empty-state` Etch elements no longer flash visible-then-hidden on loops that have results. The default-hide CSS rule was previously injected at runtime by `assets/js/empty-state.js` on DOMContentLoaded, so authored empty-state elements painted with normal Etch styling for the ~50-200ms before JS ran. Bridge now emits the same `.jsf-etch-empty-state:not(.is-active){display:none !important}` rule inline in `<head>` so it applies before the browser paints any body content. The `<style>` ID matches the runtime injector's, so the JS-side idempotency check skips re-injection — no double rule, no specificity surprises. New filter: `apply_filters( 'jqbeb_empty_state_default_hide_enabled', true )` for sites that ship their own hiding mechanism (visibility, opacity, off-screen positioning, server-side Etch Conditions).
* Internal: new public helpers on `JSF_Bridge` — `capture_render_context()` and `build_context_query( array $context )`. Snapshot allowlist of `is_*` flags / `query_vars` keys is intentionally hardcoded; if a future JE / third-party block needs additional state, extend the allowlist in code with a CHANGELOG note rather than opening a filter (would invite drift between what gets captured and what `build_context_query` restores).
* Upgrade notes: drop-in upgrade; the 1.2.x transient cache shape (`{ block, post_id }`) is still honoured for the 1h TTL after the bump, so in-flight AJAX requests during deploy don't error. New writes always include `context`. The legacy `post_id` field is kept for backward compat but no longer drives non-singular restoration.
...
* Security: `sslverify` for the loopback now defaults to `true`. Local-dev override via `add_filter( 'jqbeb_loopback_sslverify', '__return_false' );` (filter name unchanged).
* Security: `redirection` for the loopback dropped from `3` to `0`. Loopback to ourselves never legitimately needs to follow redirects, and disallowing them blocks any open redirect (or 3rd-party redirect reachable from `home_url()`) from ferrying the request — and the forwarded cookies — off-site.
